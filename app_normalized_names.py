...
# Normalize names
station_data["start_station_name"] = station_data["start_station_name"].str.strip()
...
with tabs[3]:
    ...
    try:
        station_data["start_station_name"] = station_data["start_station_name"].fillna("Unknown").str.strip()
        top = station_data["start_station_name"].value_counts().reset_index()
        top.columns = ["Station Name", "Trip Count"]
        top["Station Name"] = top["Station Name"].str.strip()
        top10 = top.head(10)
        ...
        st.write("🔍 Debug: Top 10 station coordinates preview")
        st.dataframe(coord_grouped)

        if not coord_grouped.empty:
            ...
            st.warning("No station coordinates available for this filter.")
